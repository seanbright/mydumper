...
static GOptionEntry entries[] = {
	{"database", 'B', 0, G_OPTION_ARG_STRING, &db, "Database to dump",
	 NULL},
	...
	{"triggers", 'G', 0, G_OPTION_ARG_NONE, &dump_triggers, "Dump triggers",
	 NULL},
	{"events", 'E', 0, G_OPTION_ARG_NONE, &dump_events, "Dump events",
	 NULL},
...
guint64 estimate_count(MYSQL * conn, char *database, char *table, char *field,
		       char *from, char *to);
void dump_table_data_file(MYSQL * conn, char *database, char *table,
...
void set_verbose(guint verbosity)
{
	if (logfile) {
		...
		if (!logoutfile) {
			g_critical
			    ("Could not open log file '%s' for writing: %d",
			     logfile, errno);
			exit(EXIT_FAILURE);
		}
	}
...
void read_tables_skiplist(const gchar * filename)
{
	...
	do {
		g_io_channel_read_line(tables_skiplist_channel, &buf, NULL,
				       NULL, NULL);
		if (buf) {
			g_strchomp(buf);
...
void write_snapshot_info(MYSQL * conn, FILE * file)
{
	...
	while (slave && (row = mysql_fetch_row(slave))) {
		...
		for (i = 0; i < mysql_num_fields(slave); i++) {
			if (isms
			    && !strcasecmp("connection_name", fields[i].name))
				connname = row[i];
			if (!strcasecmp("exec_master_log_pos", fields[i].name)) {
				slavepos = row[i];
			} else
			    if (!strcasecmp
				("relay_master_log_file", fields[i].name)) {
				slavelog = row[i];
			} else if (!strcasecmp("master_host", fields[i].name)) {
				slavehost = row[i];
			} else
			    if (!strcasecmp("Executed_Gtid_Set", fields[i].name)
				|| !strcasecmp("Gtid_Slave_Pos",
					       fields[i].name)) {
				slavegtid = row[i];
			}
		}
...
void *process_queue(struct thread_data *td)
{
	...
	if (!mysql_real_connect
	    (thrconn, hostname, username, password, NULL, port, socket_path,
	     0)) {
		g_critical("Failed to connect to database: %s",
			   mysql_error(thrconn));
		exit(EXIT_FAILURE);
	} else {
	...
	if ((detected_server == SERVER_TYPE_MYSQL)
		...
		g_warning("Failed to increase wait_timeout: %s",
			  mysql_error(thrconn));
	}
	if (mysql_query
	    (thrconn,
	     "SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ")) {
		g_critical("Failed to set isolation level: %s",
			   mysql_error(thrconn));
		exit(EXIT_FAILURE);
	}
	if (mysql_query
		...
		g_critical("Failed to start consistent snapshot: %s",
			   mysql_error(thrconn));
		exit(EXIT_FAILURE);
	}
	if (!skip_tz
	    && mysql_query(thrconn, "/*!40103 SET TIME_ZONE='+00:00' */")) {
	...
	for (;;) {
		...
		case JOB_DUMP:
			...
			if (tj->where)
				g_message
				    ("Thread %d dumping data for `%s`.`%s` where %s",
				     td->thread_id, tj->database, tj->table,
				     tj->where);
			else
				g_message
				    ("Thread %d dumping data for `%s`.`%s`",
				     td->thread_id, tj->database, tj->table);
			if (use_savepoints
			    && mysql_query(thrconn, "SAVEPOINT mydumper")) {
				g_critical("Savepoint failed: %s",
					   mysql_error(thrconn));
			}
			dump_table_data_file(thrconn, tj->database, tj->table,
					     tj->where, tj->filename);
			if (use_savepoints
			    && mysql_query(thrconn,
					   "ROLLBACK TO SAVEPOINT mydumper")) {
				g_critical("Rollback to savepoint failed: %s",
					   mysql_error(thrconn));
			}
			if (tj->database)
				g_free(tj->database);
		...
		case JOB_SCHEMA:
			...
			g_message("Thread %d dumping schema for `%s`.`%s`",
				  td->thread_id, sj->database, sj->table);
			dump_schema_data(thrconn, sj->database, sj->table,
					 sj->filename);
			if (sj->database)
				g_free(sj->database);
			if (sj->table)
		...
		case JOB_VIEW:
			...
			g_message("Thread %d dumping view for `%s`.`%s`",
				  td->thread_id, vj->database, vj->table);
			dump_view_data(thrconn, vj->database, vj->table,
				       vj->filename, vj->filename2);
			if (vj->database)
				g_free(vj->database);
			if (vj->table)
				g_free(vj->table);
		...
		case JOB_TRIGGERS:
			sj = (struct schema_job *)job->job_data;
			g_message("Thread %d dumping triggers for `%s`.`%s`",
				  td->thread_id, sj->database, sj->table);
			dump_triggers_data(thrconn, sj->database, sj->table,
					   sj->filename);
			if (sj->database)
				g_free(sj->database);
			if (sj->table)
		...
		case JOB_SCHEMA_POST:
			...
			g_message("Thread %d dumping SP and VIEWs for `%s`",
				  td->thread_id, sp->database);
			dump_schema_post_data(thrconn, sp->database,
					      sp->filename);
			if (sp->database)
				g_free(sp->database);
			if (sp->filename)
		...
		case JOB_BINLOG:
			...
			g_message("Thread %d dumping binary log file %s",
				  td->thread_id, bj->filename);
			get_binlog_file(thrconn, bj->filename, binlog_directory,
					bj->start_position, bj->stop_position,
					FALSE);
			if (bj->filename)
				g_free(bj->filename);
			g_free(bj);
...
void *process_queue_less_locking(struct thread_data *td)
{
	...
	for (;;) {
		...
		case JOB_LOCK_DUMP_NON_INNODB:
			...
			for (glj = g_list_first(glj); glj;
			     glj = g_list_next(glj)) {
				tj = (struct table_job *)glj->data;
				if (first) {
					g_string_printf(query,
							"LOCK TABLES `%s`.`%s` READ LOCAL",
							tj->database,
							tj->table);
					first = 0;
				} else {
					if (g_ascii_strcasecmp
					    (prev_database->str, tj->database)
					    ||
					    g_ascii_strcasecmp(prev_table->str,
							       tj->table)) {
				...
				g_string_printf(prev_database, "%s",
						tj->database);
			}
			first = 1;
			if (mysql_query(thrconn, query->str)) {
				g_critical("Non Innodb lock tables fail: %s",
					   mysql_error(thrconn));
				exit(EXIT_FAILURE);
			}
			if (g_atomic_int_dec_and_test(&non_innodb_table_counter)
			...
			for (mj->table_job_list =
			     g_list_first(mj->table_job_list);
			     mj->table_job_list;
			     mj->table_job_list =
			     g_list_next(mj->table_job_list)) {
				tj = (struct table_job *)mj->
				    table_job_list->data;
				if (tj->where)
					g_message
					    ("Thread %d dumping data for `%s`.`%s` where %s",
					     td->thread_id, tj->database,
					     tj->table, tj->where);
				else
					g_message
					    ("Thread %d dumping data for `%s`.`%s`",
...
MYSQL *reconnect_for_binlog(MYSQL * thrconn)
{
	...
	if (!mysql_real_connect
	    (thrconn, hostname, username, password, NULL, port, socket_path,
	     0)) {
		g_critical("Failed to re-connect to database: %s",
			   mysql_error(thrconn));
		exit(EXIT_FAILURE);
	}
	return thrconn;
...
int main(int argc, char *argv[])
{
	...
	GOptionGroup *main_group =
	    g_option_group_new("main", "Main Options", "Main Options", NULL,
			       NULL);
	...
	if (trx_consistency_only)
		g_warning
		    ("Using trx_consistency_only, binlog coordinates will not be accurate if you are writing to non transactional tables.");

	if (!output_directory)
		output_directory =
		    g_strdup_printf("%s-%04d%02d%02d-%02d%02d%02d", DIRECTORY,
	...
	if (daemon_mode) {
		...
		char *dump_directory =
		    g_strdup_printf("%s/0", output_directory);
	...
	if (need_binlogs) {
		binlog_directory =
		    g_strdup_printf("%s/%s", output_directory,
				    BINLOG_DIRECTORY);
	...
	if (daemon_mode) {
...
#ifdef WITH_BINLOG
		GThread *bthread =
		    g_thread_create(binlog_thread, GINT_TO_POINTER(1), FALSE,
				    &terror);
		if (bthread == NULL) {
			g_critical("Could not create binlog thread: %s",
				   terror->message);
			g_error_free(terror);
			exit(EXIT_FAILURE);
		}
		...
		GThread *ethread =
		    g_thread_create(exec_thread, GINT_TO_POINTER(1), FALSE,
				    &terror);
		if (ethread == NULL) {
			g_critical("Could not create exec thread: %s",
				   terror->message);
			g_error_free(terror);
			exit(EXIT_FAILURE);
		}
		...
		g_timeout_add(snapshot_interval * 60 * 1000,
			      (GSourceFunc) run_snapshot, NULL);
#else
		g_timeout_add_seconds(snapshot_interval * 60,
				      (GSourceFunc) run_snapshot, NULL);
#endif
		guint sigsource =
		    g_unix_signal_add(SIGINT, sig_triggered, NULL);
...
MYSQL *create_main_connection()
{
	...
	if (!mysql_real_connect
	    (conn, hostname, username, password, db, port, socket_path, 0)) {
		g_critical("Error connecting to database: %s",
			   mysql_error(conn));
		exit(EXIT_FAILURE);
	}
	...
	if ((detected_server == SERVER_TYPE_MYSQL)
		...
		g_warning("Failed to increase net_write_timeout: %s",
			  mysql_error(conn));
	}
...
void *exec_thread(void *data)
{
	...
	while (1) {
		...
		if (!shutdown_triggered) {
			const char *dump_symlink_source =
			    (dump_number == 0) ? "0" : "1";
			...
			if (symlink(dump_symlink_source, dump_symlink_dest) ==
			    -1) {
				g_critical
				    ("error setting last good dump symlink %s, %d",
...
void *binlog_thread(void *data)
{
	...
	if (master && (row = mysql_fetch_row(master))) {
		...
		get_binlog_file(binlog_connection, filename,
				daemon_binlog_directory, start_position, 0,
				TRUE);
		g_free(filename);
...
void start_dump(MYSQL * conn)
{
	...
	if (updated_since > 0) {
		if (daemon_mode)
			u = g_strdup_printf("%s/%d/not_updated_tables",
					    output_directory, dump_number);
		else
			u = g_strdup_printf("%s/not_updated_tables",
					    output_directory);
		nufile = g_fopen(u, "w");
		if (!nufile) {
			g_critical
			    ("Couldn't write not_updated_tables file (%d)",
			     errno);
			exit(EXIT_FAILURE);
		}
	...
	if (!no_locks) {
		if (mysql_query(conn, "SHOW PROCESSLIST")) {
			g_warning
			    ("Could not check PROCESSLIST, no long query guard enabled: %s",
			     mysql_error(conn));
		} else {
			...
			if ((tcol < 0) || (ccol < 0) || (icol < 0)) {
				g_critical
				    ("Error obtaining information from processlist");
				exit(EXIT_FAILURE);
			}
			while ((row = mysql_fetch_row(res))) {
				...
				if (row[tcol] && atoi(row[tcol]) > longquery) {
					if (killqueries) {
						if (mysql_query
						    (conn, p3 =
						     g_strdup_printf("KILL %lu",
								     atol(row
									  [icol]))))
							g_warning
							    ("Could not KILL slow query: %s",
							     mysql_error(conn));
						else
							g_warning
							    ("Killed a query that was running for %ss",
							     row[tcol]);
						g_free(p3);
					} else {
						g_critical
						    ("There are queries in PROCESSLIST running longer than %us, aborting dump,\n\t"
	...
	if (!no_locks) {
		...
		if (have_backup_locks) {
			if (mysql_query(conn, "LOCK TABLES FOR BACKUP")) {
				g_critical
				    ("Couldn't acquire LOCK TABLES FOR BACKUP, snapshots will not be consistent: %s",
				     mysql_error(conn));
				errors++;
			}
			...
			if (mysql_query(conn, "LOCK BINLOG FOR BACKUP")) {
				g_critical
				    ("Couldn't acquire LOCK BINLOG FOR BACKUP, snapshots will not be consistent: %s",
				     mysql_error(conn));
				errors++;
			}
			...
			if (db) {
				g_string_printf(query,
						"SELECT TABLE_SCHEMA, TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = '%s' AND TABLE_TYPE ='BASE TABLE' AND NOT (TABLE_SCHEMA = 'mysql' AND (TABLE_NAME = 'slow_log' OR TABLE_NAME = 'general_log'))",
						db);
			} else if (tables) {
				for (i = 0; tables[i] != NULL; i++) {
					dt = g_strsplit(tables[i], ".", 0);
					dbtb =
					    g_strdup_printf("`%s`.`%s`", dt[0],
							    dt[1]);
					tables_lock =
					    g_list_append(tables_lock, dbtb);
				}
			} else {
				g_string_printf(query,
						"SELECT TABLE_SCHEMA, TABLE_NAME FROM information_schema.TABLES WHERE TABLE_TYPE ='BASE TABLE' AND TABLE_SCHEMA NOT IN ('information_schema', 'performance_schema', 'data_dictionary') AND NOT (TABLE_SCHEMA = 'mysql' AND (TABLE_NAME = 'slow_log' OR TABLE_NAME = 'general_log'))");
			}

			if (tables_lock == NULL) {
				if (mysql_query(conn, query->str)) {
					g_critical
					    ("Couldn't get table list for lock all tables: %s",
					     mysql_error(conn));
					errors++;
				} else {
					MYSQL_RES *res =
					...
					while ((row = mysql_fetch_row(res))) {
						...
						if (lock && tables_skiplist_file
						    && check_skiplist(row[0],
								      row[1]))
							continue;
						if (lock && regexstring
						    && !check_regex(row[0],
			...
			while (!success && retry < 4) {
				n = 0;
				iter = tables_lock;
				for (iter = g_list_first(iter); iter;
				...
				if (mysql_query(conn, query->str)) {
					...
					tmp_fail =
					    g_strsplit(mysql_error(conn), "'",
						       0);
					tmp_fail =
					    g_strsplit(tmp_fail[1], ".", 0);
					failed_table =
					    g_strdup_printf("`%s`.`%s`",
							    tmp_fail[0],
							    tmp_fail[1]);
			...
			if (!success) {
				g_critical("Lock all tables fail: %s",
					   mysql_error(conn));
				exit(EXIT_FAILURE);
			}
			g_free(query->str);
			...
			if (mysql_query(conn, "FLUSH TABLES WITH READ LOCK")) {
				g_critical
				    ("Couldn't acquire global lock, snapshots will not be consistent: %s",
				     mysql_error(conn));
				errors++;
			}
		}
	...
	if (less_locking) {
		...
		for (n = num_threads; n < num_threads * 2; n++) {
			td[n].conf = &conf;
			td[n].thread_id = n + 1;
			threads[n] = g_thread_create((GThreadFunc)
						     process_queue_less_locking,
						     &td[n], TRUE, NULL);
	...
	for (n = 0; n < num_threads; n++) {
		...
		td[n].thread_id = n + 1;
		threads[n] =
		    g_thread_create((GThreadFunc) process_queue, &td[n], TRUE,
				    NULL);
	...
	if (db) {
		...
		if (mysql_query(conn, "SHOW DATABASES")
			...
			g_critical("Unable to list databases: %s",
				   mysql_error(conn));
			exit(EXIT_FAILURE);
		}
	...
	if (less_locking) {
		...
		for (non_innodb_table = g_list_first(non_innodb_table);
		     non_innodb_table;
		     non_innodb_table = g_list_next(non_innodb_table)) {
		...
		for (non_innodb_table = g_list_first(non_innodb_table);
		     ...
		     non_innodb_table = g_list_next(non_innodb_table)) {
			dbt = (struct db_table *)non_innodb_table->data;
			dump_table(conn, dbt->database, dbt->table, &conf,
				   FALSE);
			g_atomic_int_inc(&non_innodb_table_counter);
		}
		g_list_free(g_list_first(non_innodb_table));
	...
	for (innodb_tables = g_list_first(innodb_tables); innodb_tables;
	     innodb_tables = g_list_next(innodb_tables)) {
	...
	for (table_schemas = g_list_first(table_schemas); table_schemas;
	     table_schemas = g_list_next(table_schemas)) {
	...
	for (view_schemas = g_list_first(view_schemas); view_schemas;
	     view_schemas = g_list_next(view_schemas)) {
	...
	for (schema_post = g_list_first(schema_post); schema_post;
	     schema_post = g_list_next(schema_post)) {
...
void dump_create_database(MYSQL * conn, char *database)
{
	...
	if (daemon_mode)
		filename =
		    g_strdup_printf("%s/%d/%s-schema-create.sql%s",
				    output_directory, dump_number, database,
				    (compress_output ? ".gz" : ""));
	else
		filename =
		    g_strdup_printf("%s/%s-schema-create.sql%s",
	...
	if (!outfile) {
		g_critical("Error: DB: %s Could not create output file %s (%d)",
			   database, filename, errno);
		errors++;
		return;
	}
	...
	if (mysql_query(conn, query) || !(result = mysql_use_result(conn))) {
		if (success_on_1146 && mysql_errno(conn) == 1146) {
			g_warning("Error dumping create database (%s): %s",
				  database, mysql_error(conn));
		} else {
			g_critical("Error dumping create database (%s): %s",
				   database, mysql_error(conn));
			errors++;
		}
		g_free(query);
...
void get_not_updated(MYSQL * conn)
{
	...
	gchar *query =
	    g_strdup_printf
	    ("SELECT CONCAT(TABLE_SCHEMA,'.',TABLE_NAME) FROM information_schema.TABLES WHERE UPDATE_TIME < NOW() - INTERVAL %d DAY",
	     updated_since);
...
gboolean detect_generated_fields(MYSQL * conn, char *database, char *table)
{
	...
	gchar *query =
	    g_strdup_printf
	    ("select COLUMN_NAME from information_schema.COLUMNS where TABLE_SCHEMA='%s' and TABLE_NAME='%s' and extra like '%%GENERATED%%'",
	     database, table);
	mysql_query(conn, query);
	g_free(query);
...
GString *get_insertable_fields(MYSQL * conn, char *database, char *table)
{
	...
	gchar *query =
	    g_strdup_printf
	    ("select COLUMN_NAME from information_schema.COLUMNS where TABLE_SCHEMA='%s' and TABLE_NAME='%s' and extra not like '%%GENERATED%%'",
	     database, table);
	mysql_query(conn, query);
	g_free(query);
			    ...
			    struct configuration * conf)
{
	...
	gchar *query =
	    g_strdup_printf("SHOW INDEX FROM `%s`.`%s`", database, table);
	mysql_query(conn, query);
	g_free(query);
	...
	mysql_query(conn, query =
		    ...
		    ("SELECT %s MIN(`%s`),MAX(`%s`) FROM `%s`.`%s`",
		     (detected_server ==
		      SERVER_TYPE_MYSQL) ? "/*!40001 SQL_NO_CACHE */" : "",
	...
	case MYSQL_TYPE_INT24:
		...
		while (cutoff <= nmax) {
			chunks =
			    g_list_append(chunks,
					  g_strdup_printf
					  ("%s%s%s%s(`%s` >= %llu AND `%s` < %llu)",
					   !showed_nulls ? "`" : "",
					   !showed_nulls ? field : "",
					   !showed_nulls ? "`" : "",
		       ...
		       char *from, char *to)
{
	...
	querybase =
	    g_strdup_printf("EXPLAIN SELECT `%s` FROM `%s`.`%s`",
			    (field ? field : "*"), database, table);
	if (from || to) {
		g_assert(field != NULL);
		...
		if (from) {
			...
			mysql_real_escape_string(conn, escaped, from,
						 strlen(from));
			fromclause =
			    g_strdup_printf(" `%s` >= \"%s\" ", field, escaped);
			g_free(escaped);
		}
		if (to) {
		...
		query =
		    g_strdup_printf("%s WHERE `%s` %s %s", querybase,
				    (from ? fromclause : ""),
				    ((from
				      && to) ? "AND" : ""),
				    (to ? toclause : ""));
	...
	if (ret) {
		g_warning("Unable to get estimates for %s.%s: %s", database,
	...
	if (g_mkdir(new_directory, 0700) == -1) {
		if (errno != EEXIST) {
			g_critical("Unable to create `%s': %s",
				   new_directory, g_strerror(errno));
			exit(EXIT_FAILURE);
		}
	}
}
		   ...
		   struct configuration *conf)
{
	...
	if (detected_server == SERVER_TYPE_MYSQL)
		...
		query =
		    g_strdup_printf
		    ("SELECT TABLE_NAME, ENGINE, TABLE_TYPE as COMMENT FROM DATA_DICTIONARY.TABLES WHERE TABLE_SCHEMA='%s'",
		     database);

	if (mysql_query(conn, (query))) {
		g_critical("Error: DB: %s - Could not execute query: %s",
			   database, mysql_error(conn));
		errors++;
		g_free(query);
		return;
	...
	if (!result) {
		g_critical("Could not list tables for %s: %s", database,
			   mysql_error(conn));
		errors++;
		return;
	}
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (!is_view && row[ecol] == NULL) {
			g_warning("Broken table detected, please review: %s.%s",
				  database, row[0]);
			dump = 0;
		}
		...
		if (dump && ignore && !is_view) {
			for (i = 0; ignore[i] != NULL; i++) {
				if (g_ascii_strcasecmp(ignore[i], row[ecol]) ==
				    0) {
					dump = 0;
					break;
				}
		...
		if (g_ascii_strcasecmp(database, "mysql") == 0
			...
			|| g_ascii_strcasecmp(row[0],
					      "innodb_table_stats") == 0)) {
			dump = 0;
			continue;
		}
		...
		if (no_updated_tables && !is_view) {
			...
			for (iter = g_list_first(iter); iter;
				...
				if (g_ascii_strcasecmp
				    (iter->data,
				     g_strdup_printf("%s.%s", database,
						     row[0])) == 0) {
					g_message("NO UPDATED TABLE: %s.%s",
						  database, row[0]);
					fprintf(file, "%s.%s\n", database,
						row[0]);
					dump = 0;
		...
		if (!is_view) {
			...
			if (!no_data) {
				if (row[ecol] != NULL
				    && g_ascii_strcasecmp("MRG_MYISAM",
							  row[ecol])) {
					if (trx_consistency_only) {
						dump_table(conn, dbt->database,
							   dbt->table, conf,
							   TRUE);
					} else if (row[ecol] != NULL
						   &&
						   !g_ascii_strcasecmp("InnoDB",
								       row
								       [ecol]))
					{
						innodb_tables =
						    g_list_append(innodb_tables,
								  dbt);
					} else if (row[ecol] != NULL
						   &&
						   !g_ascii_strcasecmp("TokuDB",
								       row
								       [ecol]))
					{
						innodb_tables =
						...
						non_innodb_table =
						    g_list_append
						    (non_innodb_table, dbt);
					}
				}
			}
			if (!no_schemas) {
				table_schemas =
				    g_list_append(table_schemas, dbt);
			}
		} else {
			if (!no_schemas) {
	...
	if (dump_routines) {
		//SP
		query =
		    g_strdup_printf("SHOW PROCEDURE STATUS WHERE Db = '%s'",
				    database);
		if (mysql_query(conn, (query))) {
			g_critical
			    ("Error: DB: %s - Could not execute query: %s",
			     database, mysql_error(conn));
			errors++;
			g_free(query);
			return;
		...
		if (!post_dump) {
			...
			query =
			    g_strdup_printf
			    ("SHOW FUNCTION STATUS WHERE Db = '%s'", database);
			if (mysql_query(conn, (query))) {
				g_critical
				    ("Error: DB: %s - Could not execute query: %s",
				     database, mysql_error(conn));
			...
			while ((row = mysql_fetch_row(result)) && !post_dump) {
				/* Checks skip list on 'database.sp' string */
				if (tables_skiplist_file
				    && check_skiplist(database, row[1]))
					continue;
				/* Checks PCRE expressions on 'database.sp' string */
				if (regexstring
				    && !check_regex(database, row[1]))
					continue;
...
void get_tables(MYSQL * conn, struct configuration *conf)
{
	...
	for (x = 0; tables[x] != NULL; x++) {
		...
		query =
		    g_strdup_printf("SHOW TABLE STATUS FROM %s LIKE '%s'",
				    dt[0], dt[1]);

		if (mysql_query(conn, (query))) {
			g_critical
			    ("Error: DB: %s - Could not execute query: %s",
			     dt[0], mysql_error(conn));
			errors++;
			return;
		}
		...
		if (!result) {
			g_warning("Could not list table for %s.%s: %s", dt[0],
				  dt[1], mysql_error(conn));
			errors++;
			return;
		}
		...
		while ((row = mysql_fetch_row(result))) {
			...
			if (!row[6])
				...
				dbt->datalength =
				    g_ascii_strtoull(row[6], NULL, 10);
			if (!is_view) {
				if (trx_consistency_only) {
					dump_table(conn, dbt->database,
						   dbt->table, conf, TRUE);
					...
					("InnoDB", row[ecol])) {
					innodb_tables =
					    g_list_append(innodb_tables, dbt);
				} else
				    if (!g_ascii_strcasecmp
					("TokuDB", row[ecol])) {
					innodb_tables =
					...
					non_innodb_table =
					    g_list_append(non_innodb_table,
							  dbt);
				}
				if (!no_schemas) {
					table_schemas =
				...
				if (!no_schemas) {
					view_schemas =
					    g_list_append(view_schemas, dbt);
				}
			}
		}
	}
...
void dump_schema_post_data(MYSQL * conn, char *database, char *filename)
{
	...
	if (dump_routines) {
		// get functions
		query =
		    g_strdup_printf("SHOW FUNCTION STATUS WHERE Db = '%s'",
				    database);
		if (mysql_query(conn, query)
		    || !(result = mysql_store_result(conn))) {
			if (success_on_1146 && mysql_errno(conn) == 1146) {
				g_warning("Error dumping functions from %s: %s",
					  database, mysql_error(conn));
			} else {
				g_critical
				    ("Error dumping functions from %s: %s",
				     database, mysql_error(conn));
				errors++;
			}
			g_free(query);
		...
		while ((row = mysql_fetch_row(result))) {
			...
			if (!write_data((FILE *) outfile, statement)) {
				g_critical
				    ("Could not write stored procedure data for %s.%s",
				     database, row[1]);
				errors++;
				return;
			}
			...
			query =
			    g_strdup_printf("SHOW CREATE FUNCTION `%s`.`%s`",
					    database, row[1]);
			mysql_query(conn, query);
			...
			if (!write_data((FILE *) outfile, statement)) {
				g_critical
				    ("Could not write function data for %s.%s",
				     database, row[1]);
				errors++;
				return;
			}
		...
		if (mysql_query(conn, query)
			...
			if (success_on_1146 && mysql_errno(conn) == 1146) {
				g_warning
				    ("Error dumping stored procedures from %s: %s",
				     database, mysql_error(conn));
			} else {
				g_critical
				    ("Error dumping stored procedures from %s: %s",
		...
		while ((row = mysql_fetch_row(result))) {
			...
			g_string_set_size(statement, 0);
			query =
			    g_strdup_printf("SHOW CREATE PROCEDURE `%s`.`%s`",
					    database, row[1]);
			mysql_query(conn, query);
			result2 = mysql_store_result(conn);
			row2 = mysql_fetch_row(result2);
	...
	if (dump_events) {
		...
		if (mysql_query(conn, query)
		    || !(result = mysql_store_result(conn))) {
			if (success_on_1146 && mysql_errno(conn) == 1146) {
				g_warning("Error dumping events from %s: %s",
					  database, mysql_error(conn));
			} else {
				g_critical("Error dumping events from %s: %s",
					   database, mysql_error(conn));
				errors++;
			}
			g_free(query);
		...
		while ((row = mysql_fetch_row(result))) {
			...
			query =
			    g_strdup_printf("SHOW CREATE EVENT `%s`.`%s`",
					    database, row[1]);
			mysql_query(conn, query);
			result2 = mysql_store_result(conn);
			...
			if (!write_data((FILE *) outfile, statement)) {
				g_critical
				    ("Could not write event data for %s.%s",
				     database, row[1]);
				errors++;
				return;
			}
			...
			char *filename)
{
	...
	// get triggers
	query =
	    g_strdup_printf("SHOW TRIGGERS FROM `%s` LIKE '%s'", database,
			    table);
	if (mysql_query(conn, query) || !(result = mysql_store_result(conn))) {
		if (success_on_1146 && mysql_errno(conn) == 1146) {
			g_warning("Error dumping triggers (%s.%s): %s",
				  database, table, mysql_error(conn));
		} else {
			g_critical("Error dumping triggers (%s.%s): %s",
				   database, table, mysql_error(conn));
			errors++;
		}
		g_free(query);
	...
	while ((row = mysql_fetch_row(result))) {
		set_charset(statement, row[8], row[9]);
		if (!write_data((FILE *) outfile, statement)) {
			g_critical("Could not write triggers data for %s.%s",
				   database, table);
			errors++;
			return;
		}
		g_string_set_size(statement, 0);
		query =
		    g_strdup_printf("SHOW CREATE TRIGGER `%s`.`%s`", database,
				    row[0]);
		mysql_query(conn, query);
		result2 = mysql_store_result(conn);
		row2 = mysql_fetch_row(result2);
...
void dump_schema_data(MYSQL * conn, char *database, char *table, char *filename)
{
	...
	if (!write_data((FILE *) outfile, statement)) {
		g_critical("Could not write schema data for %s.%s", database,
			   table);
		errors++;
		return;
	}
	...
	if (mysql_query(conn, query) || !(result = mysql_use_result(conn))) {
		if (success_on_1146 && mysql_errno(conn) == 1146) {
			g_warning("Error dumping schemas (%s.%s): %s", database,
				  table, mysql_error(conn));
		} else {
			g_critical("Error dumping schemas (%s.%s): %s",
				   database, table, mysql_error(conn));
			errors++;
		}
		g_free(query);
		    ...
		    char *filename2)
{
	...
	if (!outfile || !outfile2) {
		g_critical("Error: DB: %s Could not create output file (%d)",
			   database, errno);
		errors++;
		return;
	}
	...
	if (!write_data((FILE *) outfile, statement)) {
		g_critical("Could not write view schema for %s.%s", database,
			   table);
		errors++;
	}
	//real view
			  ...
			  char *where, char *filename)
{
	...
	if (!outfile) {
		g_critical
		    ("Error: DB: %s TABLE: %s Could not create output file %s (%d)",
		 ...
		 struct configuration *conf)
{
	...
	if (daemon_mode)
		sj->filename =
		    g_strdup_printf("%s/%d/%s.%s-schema.sql%s",
				    output_directory, dump_number, database,
				    table, (compress_output ? ".gz" : ""));
	else
		sj->filename =
		    g_strdup_printf("%s/%s.%s-schema.sql%s", output_directory,
				    database, table,
				    (compress_output ? ".gz" : ""));
	g_async_queue_push(conf->queue, j);
	...
	if (dump_triggers) {
		...
		query =
		    g_strdup_printf("SHOW TRIGGERS FROM `%s` LIKE '%s'",
				    database, table);
		if (mysql_query(conn, query)
		    || !(result = mysql_store_result(conn))) {
			g_critical("Error Checking triggers for %s.%s. Err: %s",
				   database, table, mysql_error(conn));
			errors++;
		} else {
			if (mysql_num_rows(result)) {
				struct job *t = g_new0(struct job, 1);
				struct schema_job *st =
				    g_new0(struct schema_job, 1);
				t->job_data = (void *)st;
	...
	if (daemon_mode) {
		vj->filename =
		    g_strdup_printf("%s/%d/%s.%s-schema.sql%s",
				    ...
				    table, (compress_output ? ".gz" : ""));
		vj->filename2 =
		    g_strdup_printf("%s/%d/%s.%s-schema-view.sql%s",
				    output_directory, dump_number, database,
				    table, (compress_output ? ".gz" : ""));
	} else {
		vj->filename =
		    g_strdup_printf("%s/%s.%s-schema.sql%s", output_directory,
				    ...
				    (compress_output ? ".gz" : ""));
		vj->filename2 =
		    g_strdup_printf("%s/%s.%s-schema-view.sql%s",
				    output_directory, database, table,
				    (compress_output ? ".gz" : ""));
	}
	g_async_queue_push(conf->queue, j);
...
void dump_schema_post(char *database, struct configuration *conf)
{
	...
	if (daemon_mode) {
		sp->filename =
		    g_strdup_printf("%s/%d/%s-schema-post.sql%s",
				    output_directory, dump_number, database,
				    (compress_output ? ".gz" : ""));
	} else {
		sp->filename =
		    g_strdup_printf("%s/%s-schema-post.sql%s", output_directory,
		...
		struct configuration *conf, gboolean is_innodb)
{
	...
	if (chunks) {
		int nchunk = 0;
		for (chunks = g_list_first(chunks); chunks;
			...
			if (daemon_mode)
				tj->filename =
				    g_strdup_printf("%s/%d/%s.%s.%05d.sql%s",
						    output_directory,
						    dump_number, database,
						    table, nchunk,
						    (compress_output ? ".gz" :
						     ""));
			else
				tj->filename =
				    g_strdup_printf("%s/%s.%s.%05d.sql%s",
						    output_directory, database,
						    table, nchunk,
						    (compress_output ? ".gz" :
						     ""));
		...
		if (daemon_mode)
			tj->filename =
			    g_strdup_printf("%s/%d/%s.%s%s.sql%s",
					    output_directory, dump_number,
					    database, table,
		 ...
		 struct configuration *conf)
{
	...
	for (noninnodb_tables_list = g_list_first(noninnodb_tables_list);
	     noninnodb_tables_list;
	     noninnodb_tables_list = g_list_next(noninnodb_tables_list)) {
		dbt = (struct db_table *)noninnodb_tables_list->data;

		if (rows_per_file)
			chunks =
			    get_chunks_for_table(conn, dbt->database,
						 dbt->table, conf);
		...
		if (chunks) {
			...
			for (chunks = g_list_first(chunks); chunks;
			     chunks = g_list_next(chunks)) {
				struct table_job *tj =
				    g_new0(struct table_job, 1);
				tj->database =
				    g_strdup_printf("%s", dbt->database);
				...
				if (daemon_mode)
					tj->filename =
					    ...
					    ("%s/%d/%s.%s.%05d.sql%s",
					     output_directory, dump_number,
					     dbt->database, dbt->table, nchunk,
					     (compress_output ? ".gz" : ""));
				else
					tj->filename =
					    g_strdup_printf
					    ("%s/%s.%s.%05d.sql%s",
					     output_directory, dbt->database,
					     dbt->table, nchunk,
					     (compress_output ? ".gz" : ""));
				tj->where = (char *)chunks->data;
				tjs->table_job_list =
				    g_list_append(tjs->table_job_list, tj);
				nchunk++;
			}
		} else {
			struct table_job *tj = g_new0(struct table_job, 1);
			...
			char *where, char *filename)
{
	...
	query =
	    g_strdup_printf("SELECT %s %s FROM `%s`.`%s` %s %s",
			    (detected_server ==
			     SERVER_TYPE_MYSQL) ? "/*!40001 SQL_NO_CACHE */" :
			    "", select_fields->str, database, table,
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (!statement->len) {
			if (!st_in_file) {
				...
				if (!write_data(file, statement)) {
					g_critical
					    ("Could not write out data for %s.%s",
					     database, table);
					return num_rows;
				}
			}
		...
		for (i = 0; i < num_fields; i++) {
			...
			if (!row[i]) {
				...
				mysql_real_escape_string(conn, escaped->str,
							 row[i], lengths[i]);
			...
			if (i < num_fields - 1) {
				...
				if (statement->len + statement_row->len + 1 >
				    statement_size) {
					if (num_rows_st == 0) {
						g_string_append(statement,
								statement_row->str);
						g_string_set_size(statement_row,
								  0);
						g_warning
						    ("Row bigger than statement_size for %s.%s",
						     database, table);
					}
					g_string_append(statement, ";\n");
	...
	if (mysql_errno(conn)) {
		g_critical("Could not read data from %s.%s: %s", database,
			   table, mysql_error(conn));
		errors++;
	}
 ...
 cleanup:
	...
	if (!st_in_file && !build_empty_files) {
		...
		fcfile =
		    g_strdup_printf("%s.sql%s", filename_prefix,
				    (compress_output ? ".gz" : ""));
...
gboolean write_data(FILE * file, GString * data)
{
	...
	while (written < data->len) {
		if (!compress_output)
			...
			r = gzwrite((gzFile) file, data->str + written,
				    data->len);

		if (r < 0) {
			g_critical("Couldn't write data to a file: %s",
				   strerror(errno));
			errors++;
			return FALSE;
		}
		    ...
		    const gchar * message, gpointer user_data)
{
	...
	if (write(fileno(logoutfile), message_out->str, message_out->len) <= 0) {
		fprintf(stderr,
			"Cannot write to log file with error %d.  Exiting...",
			errno);
	}
