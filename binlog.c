...
void get_binlogs(MYSQL * conn, struct configuration *conf)
{
	...
	if (mysql_query(conn, "SHOW MASTER STATUS")) {
		g_critical("Error: Could not execute query: %s",
			   mysql_error(conn));
		return;
	}
		     ...
		     guint64 stop_position, gboolean continuous)
{
	...
	if (simple_command(conn, COM_BINLOG_DUMP, buf,
		...
		g_critical
		    ("Error: binlog: Critical error whilst requesting binary log");
	}
	...
	while (1) {
		...
		if (outfile == NULL) {
			g_critical
			    ("Error: binlog: Could not create binlog file '%s', %d",
			     binlog_file, errno);
			return;
		}
		...
		while (1) {
			...
			if ((len == 0) || (len == ~(unsigned long)0)) {
				...
				if (mysql_errno(conn) ==
					...
					g_critical
					    ("Error: binlog: Network packet read error getting binlog file: %s",
					     binlog_file);
					close_binlog_file(outfile);
					return;
				}
			...
			pos_counter += len;
			event_type =
			    get_event((const char *)net->read_pos + 1, len - 1);
			switch (event_type) {
			case EVENT_TOO_SHORT:
				g_critical
				    ("Error: binlog: Event too short in binlog file: %s",
				     binlog_file);
				read_error = TRUE;
				break;
			case ROTATE_EVENT:
			...
			if (read_error)
				break;
			write_binlog(outfile, (const char *)net->read_pos + 1,
				     len - 1);
			if (read_end) {
				if (!continuous) {
					break;
...
char *rotate_file_name(const char *buf)
{
	...
	event_length =
	    event_length - EVENT_HEADER_LENGTH - EVENT_ROTATE_FIXED_LENGTH;
...
void write_binlog(FILE * file, const char *data, guint64 len)
{
	...
	if (len > 0) {
		...
		if (write_result <= 0) {
			if (!compress_output)
				g_critical
				    ("Error: binlog: Error writing binary log: %s",
				     strerror(errno));
			else
				g_critical
				    ("Error: binlog: Error writing compressed binary log: %s",
